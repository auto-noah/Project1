    ...
    if (m_intersection.Intersect(ray, 1e20, ignore, nearest, t, intersect))
    {
        ...
        for (int i = 0; i < LightCnt(); ++i)
        {
            ...
            if (!m_intersection.Intersect(shadowRay, length, nearest, shadowNearest, t, intersect))
            {
                ...
                CGrPoint specularother = CalculateIndirectSpecular(ray, N, intersect, recurse);
                color += CalculateLighting(N, material, light, lightDir, intersect, color) + specularother;
            }
        }
    }
    else
